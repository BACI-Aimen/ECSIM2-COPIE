...
exports.getUnMurUtilisateurByID = async (req, res) => {
  try {
    const { id_mur } = req.params;
    ...
    const mur = await ModelMur.getMurUtilisateur(id_mur);

    // Vérifie si le mur existe
    if (!mur) {
      return res.status(404).json({ error: "Mur non trouvé" });
    }

    return res.status(200).json(mur); // Retourne les infos du mur
  } catch (err) {
    return res.status(500).json({ error: err.message });
  }
};
exports.getMonMurUtilisateurById = async (req, res) => {
  try {
    //const { id_mur } = req.params;
